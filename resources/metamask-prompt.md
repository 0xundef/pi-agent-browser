...
6. **Network traffic (Fetch/XHR + WebSocket only):** before finishing, call **`capture_network_traffic`**. It runs `playwright-cli network --request-headers --filter="https?://"` and writes **`ai_testing/<runId>/network.json`** (always call it—even with zero API requests, the file must exist with `"requests": []`). Use it to note RPC/API hosts MetaMask contacted (e.g. Infura, chain endpoints)—do not treat static asset loads as security signals.
